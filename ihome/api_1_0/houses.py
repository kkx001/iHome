...
def get_house_index():
    ...
    if ret:
        current_app.logger.info("hit house index info")
        # 因为redis中保存的是json字符串，所以直接进行字符串拼接返回
        return '{"errno": "OK", "errmsg": "OK", "data": %s}' % ret, 200, {"Content-Type": "application/json"}
    else:
...
def get_house_list():
    ...
    end_date = request.args.get("ed", "")  # 用户想要的截止时间
    area_id = request.args.get("aid", "")  # 区域编号
    sort_key = request.args.get("sk", "new")  # 排序相关
