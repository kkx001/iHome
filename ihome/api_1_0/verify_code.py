...
from . import api
from ihome.utils.captcha_for_python3.captcha import captcha
...
def get_image_code(image_code_id):
    ...
    try:
        ...
        return jsonify(errno=RET.DBERR, errmsg="保存图片验证码失效")
...
#         #保存发送给这个手机号的记录，防止用户在60s的时间内重复操作
#         redis_store.setex("sms_code_%s" %mobile, constants.SEND_SMS_CODE_INTERVAL, sms_code)
#     except Exception as e:
#         current_app.logger.error(e)
...
def get_sms_code(mobile):
    ...
    try:
        redis_store.setex("sms_code_%s" % mobile, constants.SMS_CODE_REDIS_EXPIRES, sms_code)
        # 保存发送给这个手机号的记录，防止用户在60s的时间内重复操作
        redis_store.setex("send_sms_code_%s" % mobile, constants.SEND_SMS_CODE_INTERVAL, sms_code)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="保存验证码异常")
