...
$(document).ready(function(){
    ...
    $.get("/api/v1.0/areas", function (data) {
        if ("0" == data.errno) {
            //用户从首页跳转到这个搜索页面时可能选择了城区，所以尝试
            //从url的查询字符串参数中提取用户的选择的城区
            var areaId = queryData["aid"];
            //如果提取到了城区的id的数据
            if (areaId) {
                for (var i=0; i<data.data.length; i++) {
                    // 对于从url查询字符串参数中拿到的城区，在页面中做高亮展示
                    // 后端获取到城区id是整型，从url参数中获取到的是字符串类型，
                    // 所以将url参数中获取到的转换为整型，再进行对比
                ...
                //如果url参数中没有城区信息，不需要做额外处理，直接遍历展示在页面中
                for (var i=0; i<data.data.length; i++) {
                    $(".filter-area").append('<li area-id=" '+data.data[i].aid + ' ">' + data.data[i].aname + '</li>');
                }
            }
